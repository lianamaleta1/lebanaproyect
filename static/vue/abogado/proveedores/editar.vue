          ...
          <div class="col-sm-6" style="margin-left: 25%">
            <div class="card card-primary card-outline card-outline-tabs">
              <div class="card-header p-0 border-bottom-0">
                <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
                  <li class="nav-item">
                    <a class="nav-link active" id="custom-tabs-four-home-tab" data-toggle="pill" href="#custom-tabs-four-home" role="tab" aria-controls="custom-tabs-four-home" aria-selected="true"><i class="fa fa-plus"></i> Editar Proveedor</a>
                  </li>
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title"><i class="fa fa-list"></i> Formulario de Edición </h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
module.exports = {
  ...
  methods: {
    ...
    editar:function(){
     var url='https://'+document.location.hostname+':'+document.location.port+'/solicitud_pago/sistema/editar_tcp/'+this.tcp.id+'/';
     axios.post(url,this.tcp).then(response =>{         
      ...
      get_editar:function(){ 	
      this.errors=[];
      var url='https://'+document.location.hostname+':'+document.location.port+'/solicitud_pago/sistema/detalles_tcp/'+this.tcp.id+'/';
      axios.get(url).then(response => {
         this.tcp.id=response.data.tcp.id;
         this.tcp.division_territorial=response.data.tcp.division_territorial;
         this.tcp.importe_maximo=response.data.tcp.importe_maximo;
         this.tcp.nombre=response.data.tcp.nombre;
         this.tcp.info=response.data.tcp.info;
         this.tcp.estado=response.data.tcp.estado;
         $('#estado').val(this.tcp.estado);
